...
export function Indicator({ database }: any) {
  const { classes, cx } = useStyles();

  const items = database.map((item: any, _index: any) => (
    <Box
      key={item.subtitle}
      className={cx(classes.link, { [classes.linkActive]: _index === 2 })}
    ...
    <div>
      <div className={classes.links}>
        <div
          ...
          style={{ transform: `translateY(${2 * LINK_HEIGHT + INDICATOR_OFFSET}px)` }}
        />
        {items}
      </div>
    </div>
